...
async function request(body:object){const res=await fetch("/api/trend",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const json=await res.json();if(!res.ok)throw new Error(json.error||"请求失败");return json}
...
export default function Home(){
 ...
 return <main>
  ...
  </section><section className="panel"><div className="section-title"><span>02</span><div><h2>今日纪律摘要</h2><p>状态条件不是自动买卖指令</p></div></div><div className="summary-cards"><div><b>优先观察</b><p>右侧成立、强度靠前且处于小暑/大暑的标的，进入复核清单。</p></div><div><b>风险复核</b><p>开香槟、危险、沸及温度转弱，只提示按既定纪律检查。</p></div><div><b>BTC 代理</b><p>MSTR、ABTC 仅作风险温度辅助；BTC 现货仍需专门行情源。</p></div></div></section></>}
  ...
  <section className="panel method"><div className="section-title"><span>边界</span><div><h2>事实与判断</h2><p>每个结论都应可追溯</p></div></div><div className="boundary"><div><b>接口事实</b><p>日期、回报、温度、右侧状态、节气、风险标志与强度来自趋势动物 API。</p></div><div><b>分析判断</b><p>页面只执行排序、筛选和纪律复核，不把任何状态表述为确定性买卖结果。</p></div></div></section>
